  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        {/* Abonnement */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Mon abonnement</h2>
          {subscription ? (
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="px-2 py-1 bg-green-100 text-green-700 rounded-full text-sm font-medium">Actif</span>
              </div>
              {subscription.current_period_end && (
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">Prochaine facturation</span>
                  <span className="text-gray-900">{new Date(subscription.current_period_end).toLocaleDateString('fr-FR')}</span>
                </div>
              )}
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Limite quotidienne</span>
                <span className="text-gray-900">
                  {subscription.limits?.daily_usage === -1 ? '∞ Illimité' : `${subscription.limits?.daily_usage} / jour`}
                </span>
              </div>
            </div>
          ) : (
            ...
            <div className="flex justify-between">
              ...
              <span className="text-gray-900">
                {profile?.user?.created_at
                  ? new Date(profile.user.created_at).toLocaleDateString('fr-FR')
                  : '—'}
              </span>
            </div>
          </div>
